...
async function fetchTwitchChannelsStatus(
  ...
  const normalizedChannels = channels.map((channel) => {
    ...
    return {
      ...
      normalizedLogin,
      normalizedChannelId: isTwitchUserId(channel.channelId)
        ? channel.channelId
        : undefined,
      request: channel,
    };
  ...
  const loginsNeedingResolution = validLogins.filter((login) =>
    !loginToId.has(login)
  );

  if (loginsNeedingResolution.length > 0) {
    try {
      const resolvedUsers = await resolveTwitchUserIdsByLogin(
        loginsNeedingResolution,
        userToken,
      );
      ...
      log.warn("Twitch /helix/users failed", {
        error: truncateUpstreamLogBody(String(error)),
      });
      return normalizedChannels.map((channel) => ({
        channelLogin: channel.normalizedLogin ??
          channel.originalLogin.toLowerCase(),
        isLive: false,
  ...
  const broadcasterIds = [
    ...new Set(
      validLogins.map((login) => loginToId.get(login)).filter(Boolean),
    ),
  ] as string[];
  ...
  const loginParams = validLogins
    ...
    .join("&");
  const idParams = broadcasterIds.map((id) =>
  ...
  const [streamsRes, channelsRes] = await Promise.all(
    userToken
      ? [
        fetch(`https://api.twitch.tv/helix/streams?${loginParams}&first=100`, {
          headers,
        }),
      ...
      : [
        ...
        fetchTwitchHelixWithAppToken(
          `https://api.twitch.tv/helix/channels?${idParams}`,
        ),
      ],
  );
...
async function fetchKickChannelStatus(
  ...
  const res = await fetch(
    `https://api.kick.com/public/v1/channels?slug=${
      encodeURIComponent(channel.channelLogin)
    }`,
    {
      headers: {
  ...
  if (!res.ok) {
    log.warn("Kick channel fetch failed", {
...
async function fetchKickChannelsStatus(
  channels: ChannelStatusRequest[],
): Promise<ChannelStatus[]> {
  if (channels.length === 0) {
    return [];
  ...
  return mapWithConcurrency(
    channels,
    KICK_CHANNEL_FETCH_CONCURRENCY,
    (channel) => fetchKickChannelStatus(channel, token),
  );
}
...
function validateOptionalString(
  ...
  if (typeof value !== "string") {
    throw new InvalidChannelsStatusRequestError(
...
function validateChannelStatusRequest(
  value: unknown,
  index: number,
): ChannelStatusRequest {
  if (!isRecord(value)) {
  ...
  if (platform !== "twitch" && platform !== "kick") {
    throw new InvalidChannelsStatusRequestError(
      `channels[${index}].platform is unsupported`,
    );
  }
  ...
  if (!channelLogin || channelLogin.trim().length === 0) {
    throw new InvalidChannelsStatusRequestError(
      `channels[${index}].channelLogin is required`,
    );
  }
...
export async function handleChannelsStatus(
  ...
  if (channels.length > MAX_CHANNELS) {
    throw new InvalidChannelsStatusRequestError(
      `channels cannot exceed ${MAX_CHANNELS}`,
    );
  }
