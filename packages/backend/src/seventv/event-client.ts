...
export class SevenTVEventClient {
  ...
  private handleMessage(data: string): void {
    try {
      ...
      if (message.op !== 2) {
        const dispatchType =
          typeof message.d === "object" && message.d !== null &&
            "type" in message.d
      ...
      switch (message.op) {
        ...
        case 5: {
          ...
          if (ack.command === "RESUME") {
            log.info(
              "Session resumed successfully — server will replay missed events",
            );
            this.resumePending = false;
          }
          break;
        ...
        case 6: {
          ...
          if (this.resumePending) {
            log.warn(
              "Resume rejected by server, falling back to full resubscribe",
            );
            this.resumePending = false;
  ...
  private sendUnsubscribe(
    ...
    const payload: {
      op: number;
      d: { type: string; condition?: Record<string, string> };
    } = {
      d: { type },
      op: 36,
    };
  ...
  private scheduleReconnect(closeCode?: number): void {
    ...
    if (
      closeCode === 4001 || closeCode === 4002 || closeCode === 4003 ||
      closeCode === 4004
    ) {
