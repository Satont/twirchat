...
import {
  parseUserCardCommand,
  resolveUserCardCommand,
} from "../src/views/main/utils/chatCommands";

function createMessage(
  overrides: Partial<NormalizedChatMessage>,
...
describe("resolveUserCardCommand", () => {
  ...
  test("resolves an exact alias match", () => {
    ...
    const result = resolveUserCardCommand(
      "/user @boss",
      [createMessage({})],
      aliasMap,
    );
  ...
  test("returns not-found when no exact match exists", () => {
    expect(resolveUserCardCommand("/user @missing", [createMessage({})]))
      .toEqual({
        ok: false,
        error: "not-found",
      });
