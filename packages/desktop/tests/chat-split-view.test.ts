...
import type { ChatLayout, SplitConfig, WatchedChannel } from "@twirchat/shared";
...
function platformForSplit(
  ...
  return watchedChannelForSplit(split, watchedChannels)?.platform as
    | string
    | undefined;
}
...
function makeWatchedChannel(
  overrides: Partial<WatchedChannel> = {},
): WatchedChannel {
  return {
    id: "twitch:streamer1",
    platform: "twitch",
...
describe("watchedChannelForSplit()", () => {
  ...
  test("returns first match when multiple channels with same id", () => {
    ...
    const second = makeWatchedChannel({
      id: "twitch:x",
      displayName: "Second",
    });
...
describe("channelNameForSplit()", () => {
  ...
  test("returns displayName for matched channel split", () => {
    const split = makeSplit({ type: "channel", channelId: "twitch:streamer1" });
    const channel = makeWatchedChannel({
      id: "twitch:streamer1",
      displayName: "Streamer One",
    });
...
describe("ChatLayout structural invariants", () => {
  ...
  test("channel split type requires a channelId to be useful", () => {
    const splitWithChannel = makeSplit({
      type: "channel",
      channelId: "twitch:x",
    });
