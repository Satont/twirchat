...
import type { WatchedChannel } from "@twirchat/shared";
...
export const WatchedChannelStore = {
  findAll(): WatchedChannel[] {
    ...
    const rows = db
      ...
      .all() as unknown as WatchedChannelRow[];
    return rows.map(rowToWatchedChannel);
  },

  findById(id: string): WatchedChannel | null {
    const db = getDb();
    const row = db.prepare("SELECT * FROM watched_channels WHERE id = ?").get(
      id,
    ) as
      | WatchedChannelRow
      | undefined;
  ...
  upsert(
    ...
    const existing = db
      .prepare(
        "SELECT * FROM watched_channels WHERE platform = ? AND channel_slug = ?",
      )
      .get(platform, slug) as WatchedChannelRow | undefined;

    if (existing) {
      // Update displayName in case it changed
      db.prepare("UPDATE watched_channels SET display_name = ? WHERE id = ?")
        .run(
          displayName,
          existing.id,
        );
