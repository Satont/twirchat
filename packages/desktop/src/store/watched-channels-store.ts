...
export const WatchedChannelStore = {
  ...
  findById(id: string): WatchedChannel | null {
    const db = getDb();
    const row = db
      .prepare("SELECT * FROM watched_channels WHERE id = ?")
      .get(id) as WatchedChannelRow | undefined;
    return row ? rowToWatchedChannel(row) : null;
  },

  upsert(platform: WatchedChannel["platform"], channelSlug: string, displayName: string): WatchedChannel {
    const db = getDb();
