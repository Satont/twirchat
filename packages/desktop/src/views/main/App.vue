...
const watchedLiveStatuses = computed<Map<string, WatchedLiveStatus>>(() => {
    ...
    for (const ch of watchedChannels.value) {
        ...
        const status = streamStatusStore.getStatus(
            ch.platform as "twitch" | "kick",
            ch.channelSlug,
        );
        map.set(ch.id, {
...
const youtubeAuthenticated = computed(() =>
    accounts.value.some((a) => a.platform === "youtube"),
);
...
async function loadInitialData() {
    try {
        ...
        if (watched !== undefined) {
            ...
            if (
                persistedTabIds !== null &&
                persistedTabIds !== undefined &&
                persistedTabIds.length > 0
            ) {
                tabChannelIds.value = new Set(
                    persistedTabIds.filter((id) =>
                        watched.some((ch) => ch.id === id),
                    ),
                );
            } else {
                // Backward compat: first run or migration — use all watched channels
                tabChannelIds.value = new Set(watched.map((ch) => ch.id));
                await bindings.setTabChannelIds?.({
                    ids: watched.map((ch) => ch.id),
                });
            }
        }
        ...
        // Eagerly pre-populate tab channel names from persisted layouts
        {
            ...
            for (const tabId of tabChannelIds.value) {
                try {
                    const layout = await bindings.getWatchedChannelsLayout?.({
                        tabId,
                    });
                    if (layout) {
        ...
        console.warn(
            "[App] Initial data load failed, retrying in 1s...",
            error,
        );
    ...
    for (const ch of watchedChannels.value) {
        try {
            const msgs = await bindings.getWatchedChannelMessages({
                id: ch.id,
            });
...
useRpcListener(
    "auth_error",
    ({ platform, error }: { platform: string; error: string }) => {
...
useRpcListener(
    ...
    ({
        ...
        watchedStatuses.value = new Map(watchedStatuses.value).set(
            channelId,
            status,
        );
    },
...
function onTabChannelsUpdated({
    ...
    tabChannelNames.value = new Map(tabChannelNames.value).set(
        tabId,
        channelNames,
    );
}
...
async function onRemoveChannel(id: string) {
    try {
        const ch = watchedChannels.value.find(
            (c: WatchedChannel) => c.id === id,
        );
        await bindings.removeWatchedChannel({ id });
        watchedChannels.value = watchedChannels.value.filter(
            (c: WatchedChannel) => c.id !== id,
        );
        tabChannelIds.value = new Set(
            [...tabChannelIds.value].filter((i) => i !== id),
        );
        await bindings.setTabChannelIds?.({ ids: [...tabChannelIds.value] });
        watchedMessages.value = new Map(
            [...watchedMessages.value].filter(([k]) => k !== id),
        );
        watchedStatuses.value = new Map(
            [...watchedStatuses.value].filter(([k]) => k !== id),
        );
        if (ch && ch.platform !== "youtube") {
            streamStatusStore.removeChannel(
                ch.platform as "twitch" | "kick",
                ch.channelSlug,
            );
        }
        ...
        <div v-if="updateState.show" class="update-toast">
            <div class="update-content">
                ...
                <div class="update-info">
                    ...
                    <div
                        v-if="updateState.progress !== undefined"
