...
export const useLayoutStore = defineStore("layout", () => {
  ...
  const loadLayout = async (tabId: string): Promise<void> => {
    ...
    try {
      ...
      layout.value = (saved as WatchedChannelsLayout | null | undefined) ??
        null;
    } catch (e) {
  ...
  const movePanel = (
    ...
    const sourceIndex = sourceParent.children.findIndex((c) =>
      c.id === sourceId
    );
    ...
    if (
      sourceParent.node.id === targetParent.node.id && sourceIndex < targetIndex
    ) {
      targetIndex--;
    }
    ...
    if (
      ...
      sourceParent.node.id !== targetParent.node.id
    ) {
      const onlyChild = sourceParent.children[0];
      if (onlyChild) {
        const grandparent = findParentOfNode(
          layout.value.root,
          sourceParent.node.id,
        );
        if (grandparent) {
          const parentIndex = grandparent.children.findIndex((c) =>
            c.id === sourceParent.node.id
          );
          if (parentIndex !== -1) {
  ...
  const dropPanel = (
    ...
    if (!targetParent) {
      ...
      const targetIndex = targetParent.children.findIndex((c) =>
        c.id === targetId
      );
      if (targetIndex !== -1) {
