...
const isEmpty = computed(
    () => !loadingInitial.value && messages.value.length === 0 && !error.value,
);
...
<template>
    <section class="history-panel">
        ...
        <div v-if="loadingInitial" class="history-panel-state">
            Loading messages…
        </div>
        <div
            v-else-if="error"
            class="history-panel-state history-panel-state-error"
        >
            <span>{{ error }}</span>
            <button
                class="history-panel-inline-btn"
                @click="void loadInitial()"
            >
                Retry
            </button>
        </div>
        <div v-else-if="isEmpty" class="history-panel-state">
            No stored messages for this user yet.
        </div>
        <div v-else class="history-panel-list-wrap">
            <div
                v-if="hasMore || loadingOlder"
                class="history-panel-top-status"
            >
                {{
                    loadingOlder
