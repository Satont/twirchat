...
const emit = defineEmits<{
    ...
    "send-watched": [
        payload: { text: string; channelId: string; replyToMessageId?: string },
    ];
    "split-right": [];
    "change-channel": [];
    "close-panel": [];
...
const watchedStreamStatus = computed(() => {
    if (!props.watchedChannel || props.watchedChannel.platform === "youtube")
        return undefined;
...
const selectedUserCardAlias = computed(() => {
    ...
    return (
        aliasStore.getAlias(target.platform, target.platformUserId) ??
        target.currentAlias
    );
});
    ...
    for (const [, info] of props.statuses) {
        if (
            ...
            result.push({
                ...
                platform: info.platform as "twitch" | "kick",
            });
        }
    }
    return result;
...
const displayedChannels = computed<ChannelStatus[]>(() =>
    allChannels.value.map((ch) => {
        const stored = streamStatusStore.getStatus(
            ch.platform,
            ch.channelLogin,
        );
    ...
    await Promise.allSettled(
        targets.map((t) =>
            bindings
                ...
                .catch((error) =>
                    console.warn(
                        `[ChatInput] send failed on ${t.platform}:`,
                        error,
                    ),
                ),
        ),
...
function onSendWatched(payload: {
    text: string;
    channelId: string;
    replyToMessageId?: string;
}) {
...
function onAppearanceChange(s: AppSettings) {
    ...
    bindings
        .saveSettings(s)
        .catch((error) =>
            console.warn("[ChatList] saveSettings failed:", error),
        );
}
</script>
        ...
        >
            ...
            <template v-if="watchedChannel">
                ...
                <template v-if="watchedStreamStatus?.isLive">
                    <span
                        v-if="watchedStreamStatus.viewerCount !== undefined"
                        class="watched-viewers"
                    >
                        {{ formatViewers(watchedStreamStatus.viewerCount) }}
                    </span>
                    <span
                        v-if="watchedStreamStatus.categoryName"
                        class="watched-category"
                    >
                        {{ watchedStreamStatus.categoryName }}
                    </span>
                    <span
                        v-if="watchedStreamStatus.title"
                        class="watched-stream-title"
                    >
                ...
                <div v-if="displayedChannels.length > 0" class="header-chips">
                    ...
                    >
                        <div
                            ...
                            :style="{
                                '--chip-color': platformColor(ch.platform),
                            }"
                        >
                            <span
                                class="chip-dot"
                                :class="{ pulse: ch.isLive }"
                            />
                            <span class="chip-name">{{ ch.channelLogin }}</span>
                            <span
                                v-if="ch.isLive && ch.viewerCount !== undefined"
                                class="chip-viewers"
                            >
                        ...
                        <template #content>
                            <div class="chip-tooltip-header">
                                ...
                                <span
                                    class="chip-tooltip-status"
                                    :class="{ live: ch.isLive }"
                                >
                                    {{ ch.isLive ? "LIVE" : "Offline" }}
                                </span>
                            </div>
                            <div v-if="ch.title" class="chip-tooltip-title">
                                {{ ch.title }}
                            </div>
                            <div
                                v-if="ch.categoryName"
                                class="chip-tooltip-row"
                            >
                                <span class="chip-tooltip-label">Category</span
                                >{{ ch.categoryName }}
                            </div>
                            <div
                                v-if="ch.isLive && ch.viewerCount !== undefined"
                                class="chip-tooltip-row"
                            >
                                <span class="chip-tooltip-label">Viewers</span
            ...
            <div class="chat-header-right">
                ...
                <template v-if="!isMain">
                    ...
                    <div class="panel-actions">
                        ...
                        <!-- ⋮ menu button -->
                        <button
                            class="panel-action-btn panel-menu-btn"
        ...
        <div class="chat-list-wrapper">
                ...
                <template #default="{ item }">
                    <ChatMessage
                        ...
                        :show-platform-color-stripe="
                            settings?.showPlatformColorStripe
                        "
            ...
            <div v-else class="empty-state chat-list">
                ...
                <template v-if="watchedChannel">
                    ...
                    <p class="empty-hint">
                        Messages from
                        <strong>{{ watchedChannel.displayName }}</strong> will
                        appear here.
                    </p>
                </template>
                ...
                <!-- No accounts at all -->
                <template
                    v-else-if="accounts.length === 0 && !hasAnyConnection"
                >
                ...
                <template v-else-if="!hasAnyConnection">
                    ...
                    <div class="empty-accounts">
                        <div
                            ...
                            :style="{
                                '--chip-color': platformColor(acc.platform),
                            }"
                        >
                            <img
                                v-if="acc.avatarUrl"
                            ...
                            <span class="chip-platform">{{
                                acc.platform
                            }}</span>
                        </div>
                    </div>
                    <p class="empty-hint">
                        Join a channel in <strong>Platforms</strong> to see
                        chat.
                    </p>
                </template>
...
@keyframes chip-pulse {
    0% {
        box-shadow: 0 0 0 0
            color-mix(in srgb, var(--chip-color) 60%, transparent);
    }
    70% {
