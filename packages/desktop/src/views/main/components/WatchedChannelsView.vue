...
const emit = defineEmits<{
    ...
    "tab-channels-updated": [
        payload: { tabId: string; channelNames: string[] },
    ];
}>();
    ...
    const names = allPanels.value
        ...
        .map((p) => {
            const ch = props.watchedChannels?.find(
                (c) =>
                    c.id ===
                    (p.content as { type: "watched"; channelId: string })
                        .channelId,
            );
...
const handleAddAndAssign = async (
    ...
    try {
        const newChannel = await props.onAddWatchedChannel(
            platform,
            channelSlug,
        );
