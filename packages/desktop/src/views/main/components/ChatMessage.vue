...
import type { Account, NormalizedChatMessage } from "@twirchat/shared";
...
const isSelfPing = computed((): boolean => {
    ...
    const myAccount = props.accounts.find(
        (a) => a.platform === props.message.platform,
    );
...
const selfPingStyle = computed(() =>
    isSelfPing.value && props.selfPingColor
        ? { background: props.selfPingColor }
        : {},
);
...
function onMsgClick(e: MouseEvent): void {
    ...
    if (url) {
        void bindings.openExternalUrl({ url });
    }
}
...
function scopeBadgeSvg(svgString: string, badgeId: string): string {
    const scope = `${props.message.id}-${badgeId}`.replace(
        /[^a-zA-Z0-9-]/g,
        "_",
    );
...
<template>
    ...
    >
        ...
        <div v-if="message.reply" class="reply-preview compact-reply">
            ...
            <span class="reply-author">{{
                message.reply.parentAuthor.displayName
            }}</span
            >:
    ...
    -->
      ...
      -->
            <span
                v-if="
                    props.showBadges !== false && message.author.badges.length
                "
            ...
            >
                ...
                >
                    <span
                        v-if="
                            badge.imageUrl && badge.imageUrl.startsWith('<svg')
                        "
                        class="badge-svg"
                    ...
                    <img
                        v-else-if="
                            badge.imageUrl && !brokenBadges.has(badge.id)
                        "
                        :src="badge.imageUrl"
                        :alt="badge.text"
        ...
        <div style="position: absolute; right: 0; top: 0">
            <button
                class="reply-btn compact-reply-btn"
