    ...
    (): { platform: string; channelId: string } | null => {
        if (props.watchedChannel) {
            return {
                ...
                channelId: props.watchedChannel.channelSlug,
            };
        }
...
function onEmoteSelect(alias: string): void {
    ...
    if (token.mode === "emote") {
        ...
        text.value =
            text.value.slice(0, pos) + insertion + text.value.slice(pos);
...
function send() {
    ...
    const userCardCommand = resolveUserCardCommand(
        trimmed,
        props.messages ?? [],
        aliasStore.aliasMap,
    );
