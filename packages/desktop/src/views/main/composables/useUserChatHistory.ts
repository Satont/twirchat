...
function compareMessages(
  a: NormalizedChatMessage,
  b: NormalizedChatMessage,
): number {
  const timestampDiff = new Date(a.timestamp).getTime() -
...
function insertMessageInOrder(
  ...
  const insertIndex = nextMessages.findIndex((entry) =>
    compareMessages(incomingMessage, entry) < 0
  );
...
export function useUserChatHistory(
  ...
  useRpcListener("chat_message", (message) => {
    ...
    if (
      ...
      message.author.id !== platformUserId.value
    ) return;
