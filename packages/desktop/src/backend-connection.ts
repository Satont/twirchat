...
type SevenTVBackendMessage = Extract<
  BackendToDesktopMessage,
  { channelId: string }
>;
...
export class BackendConnection {
  ...
  onSystemMessage(handler: SystemMessageHandler): () => void {
    ...
    return () => {
      this.systemMessageHandlers = this.systemMessageHandlers.filter((h) =>
        h !== handler
      );
    };
  }
        ...
        switch (msg.type) {
          ...
          case "seventv_system_message": {
            log.info("7TV system message received - HANDLING", {
              ...
              action: msg.action,
              emoteAlias:
                msg.action === "added" || msg.action === "removed" ||
                  msg.action === "updated"
                  ? msg.emote.alias
                  : undefined,
            ...
            for (const handler of this.systemMessageHandlers) {
              try {
                ...
                log.error("Error in system message handler", {
                  error: String(err),
                });
              }
            }
