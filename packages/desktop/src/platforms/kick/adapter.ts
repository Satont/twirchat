...
import { KICK_API_BASE, KICK_PUSHER_WS } from "@twirchat/shared";
import { getBackendUrl } from "../../runtime-config";
import { AccountStore } from "../../store/account-store";
import { refreshKickToken } from "../../auth/kick";
import { getKickBadgeSvg } from "./badges";
import { type KickAvatarResolver, resolveKickAvatarUrl } from "./avatar-cache";
...
export class KickAdapter extends BasePlatformAdapter {
      ...
      if (tokens) {
        ...
        if (tokens.expiresAt && tokens.expiresAt < now + 300) {
          ...
          try {
            ...
            log.error("[Kick] Failed to refresh token", {
              error: String(error),
            });
          }
        }
  ...
  private async fetchChatroomId(
    ...
    const url = `${getBackendUrl()}/api/kick/chatroom?slug=${
  ...
  private async connectPusher(): Promise<void> {
    ...
    const wsUrl =
      `${KICK_PUSHER_WS}?protocol=7&client=js&version=8.4.0&flash=false`;
    const ws = new WebSocket(wsUrl);
    this.ws = ws;

    ws.addEventListener("open", () => {
      log.info(
        `[Kick] Pusher connected (${
          this.anonymous ? "anonymous" : "authenticated"
        })`,
      );
    });
    ...
    ws.addEventListener("message", (event) => {
      try {
        ...
        log.error("[Kick] Failed to parse Pusher event", {
          error: String(error),
        });
      }
    });
    ...
    switch (event.event) {
      ...
      case String.raw`App\Events\ChatMessageEvent`: {
        ...
        this.messageQueue = this.messageQueue
          ...
          .catch((error) => {
            log.error("[Kick] Failed to handle chat message", {
              error: String(error),
            });
          });
        break;
