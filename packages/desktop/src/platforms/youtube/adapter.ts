...
export class YouTubeAdapter extends BasePlatformAdapter {
  ...
  private async resolveChannelId(input: string): Promise<string> {
    ...
    if (account) {
      ...
      if (account.username === handle || account.username === handleWithoutAt) {
        log.info(
          `[YouTube] Found channel ID from authenticated account: ${account.platformUserId}`,
        );
   ...
   */
  private async resolveHandleViaBackend(
    handle: string,
  ): Promise<string | null> {
    try {
      const res = await fetch(
        `${getBackendUrl()}/api/youtube/resolve?handle=${
      ...
      if (res.ok) {
        ...
        if (body.channelId) {
          log.info(
            `[YouTube] Backend resolved handle to channel: ${body.channelId}`,
          );
        ...
        log.warn(`[YouTube] Backend resolve failed: ${res.status}`, {
          body: body.slice(0, 300),
        });
      }
    } catch (error) {
      log.warn(`[YouTube] Backend resolve request failed`, {
        error: String(error),
      });
    }
    return null;
  ...
  private async fetchLiveChatId(): Promise<string | null> {
    ...
    try {
      ...
      const videoRes = await this.fetchWithAuth(
        `${YOUTUBE_API_BASE}/videos?part=liveStreamingDetails&id=${
          encodeURIComponent(this.currentVideoId)
        }`,
      );

      if (!videoRes.ok) {
        const body = await videoRes.text();
        log.warn(`[YouTube] Video details failed: ${videoRes.status}`, {
          body: body.slice(0, 300),
        });
        return null;
      }
      ...
      const liveChatId = videoBody.items?.[0]?.liveStreamingDetails
        ?.activeLiveChatId;
      ...
      log.error(`[YouTube] Failed to fetch liveChatId`, {
        error: String(error),
      });
      return null;
    }
  ...
  private async refreshTokenIfNeeded(): Promise<boolean> {
    ...
    if (tokens.expiresAt && tokens.expiresAt < now + 300) {
      ...
      try {
        ...
        log.error("[YouTube] Failed to refresh token", {
          error: String(error),
        });
        return false;
      }
