...
import type { PlatformStatusInfo } from "@twirchat/shared";
...
export abstract class BasePlatformAdapter implements IPlatformAdapter {
  ...
  on<K extends keyof PlatformEventMap>(
    ...
    this.listeners.get(event)!.add(
      handler as PlatformEventHandler<keyof PlatformEventMap>,
    );
  }
  ...
  protected emit<K extends keyof PlatformEventMap>(
    ...
    for (const handler of handlers) {
      try {
        ...
        log.error(`[${this.platform}] Event handler error (${event})`, {
          error: String(error),
        });
      }
    }
