...
export class ChatAggregator {
  ...
  private onStatusHandlers = new Set<
    AggregatorEventHandler<PlatformStatusInfo>
  >();
  ...
  async disconnectAll(): Promise<void> {
    const promises = [...this.adapters.values()].map((a) =>
      a.disconnect().catch((error) =>
        console.error(`[Aggregator] Disconnect error:`, error)
      )
    );
    await Promise.all(promises);
  }
