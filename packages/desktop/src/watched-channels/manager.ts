...
export class WatchedChannelManager {
    ...
    if (entry) {
      ...
      sevenTVService
        .unsubscribeFromChannel(
          entry.watchedChannel.platform,
          entry.watchedChannel.channelSlug,
        )
        .catch((error) => {
      ...
      await entry.adapter.disconnect().catch((error) => {
        log.error("Error disconnecting watched channel", {
          id,
          error: String(error),
        });
      });
  ...
  private async startChannel(ch: WatchedChannel): Promise<void> {
    ...
    if (ch.platform === "kick") {
      ...
      if (
        twitchAccount &&
        twitchAccount.username.toLowerCase() === ch.channelSlug.toLowerCase()
      ) {
  ...
  private bindAdapter(entry: WatchedEntry): void {
    ...
    const onMessage = (msg: NormalizedChatMessage) => {
      const parsed = parseMessageWithEmotes(
        msg.text,
        msg.platform,
        entry.sevenTvChannelId,
      );
    ...
    adapter.on("status", onStatus); // Store references for cleanup
    (
      entry as WatchedEntry & {
        _onMessage: typeof onMessage;
        _onStatus: typeof onStatus;
      }
