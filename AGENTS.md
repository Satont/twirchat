...
src/shared/rpc.ts         — ChatrixRPCSchema + WebviewSender (общий тип для обеих сторон)
...
const rpc = defineElectrobunRPC<ChatrixRPCSchema>("bun", { handlers: { requests: {...} } });
...
export const rpc = Electroview.defineRPC<ChatrixRPCSchema>({ handlers: { requests: {}, messages: {} } });
new Electroview({ rpc });
...
└── packages/
    ...
    └── desktop/
        ...
        └── src/
            ...
            ├── shared/
            │   └── rpc.ts               ← ChatrixRPCSchema, WebviewSender
            ├── views/
            │   ├── main/
            │   │   ├── index.html
            │   │   ├── env.d.ts
